...
function createTree(p_div,p_backColor,p_contextMenu) {
    var tree = {
                ...
                if (p_parentNode==undefined) {
                    ...
                    if (p_parentNode.childNodes.length==0) {
                        if (p_parentNode.expanded) {
                            ...
                            v_img = p_parentNode.elementLi.getElementsByTagName("i")[0];
        ...
        drawNode: function(p_ulElement,p_node) {
            ...
            if (p_node.icon!=null)
                v_icon = createImgElement(null,'icon_tree '+p_node.icon.className);
            ...
            if (p_node.childNodes.length == 0) {
                v_exp_col = createImgElement('toggle_off','exp_col '+ 'el-icon-caret-bottom');
                ...
                if (p_node.expanded) {
                    ...
                    v_exp_col = createImgElement('toggle_on','exp_col '+'el-icon-caret-right');
                }
            }
                ...
                if (p_node.contextMenu!=undefined) {
                    ...
                    for (var i=0; i<v_menu.elements.length; i++) (function(i){
                        ...
                        if (v_menu.elements[i].icon!=undefined) {
                            // var v_img = createImgElement('null',v_menu.elements[i].icon.className+' el-icon-menu');
                            var v_img = createImgElement('null','menu_img el-icon-menu');
        ...
        contextMenuLi : function(p_submenu,p_ul,p_node) {
            ...
            for (var i=0; i<p_submenu.elements.length; i++) (function(i){
                ...
                if (p_submenu.elements[i].icon!=undefined) {
                    var v_img = createImgElement('null',p_submenu.elements[i].icon);
                    v_li.appendChild(v_img);
                }
...
//Create img element
function createImgElement(p_id,p_class) {
...
// window.onload=function () {
                        ...
                        elements : [
                            {
                                ...
                                action : function(node) {
                                    var newName=rename("Untitled");
                                    node.createChildNode(newName,false,' el-icon-plus',null,'context1');
                                }
                            },
                            {
                            ...
                            },
                            {
                                ...
                                action : function(node) {
                                    ...
                                    if(node.parent.name == "tree")
                                        node.parent.createNode(newName,false,' el-icon-plus',null,null,'context1');
                                    else{
                                        node.parent.createChildNode(newName,false,' el-icon-plus',null,'context1');
                                    }
                                }
                            },
                            {
    ...
    for (var i=1; i<10; i++) {
        node1 = tree.createNode('Level 0 - Node ' + i,false,' el-icon-close',null,null,'context1');
        for (var j=1; j<5; j++) {
            node2 = node1.createChildNode('Level 1 - Node ' + j, false, ' el-icon-close',null,'context1');
            for (var k=1; k<5; k++) {
                node3 = node2.createChildNode('Level 2 - Node ' + k, false, ' el-icon-close',null,'context1');
                /*for (var l=1; l<5; l++) {
                    node4 = node3.createChildNode('Level 3 - Node ' + l, false, 'src/images/key_green.png',null,'context1');
                    for (var m=1; m<5; m++) {
                        node4.createChildNode('Level 4 - Node ' + m, false, 'src/images/file.png',null,'context1');
                    }
                }*/
            }
        }
...
//Adding node after tree is already rendered
    tree.createNode('Real Time',false,'el-icon-close',null,null,'context1');
